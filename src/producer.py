"""
Smart Wastebin — PIR Producer
================================
Reads the HC-SR501 PIR sensor via lgpio, assembles JSON-LD Observation
records, and publishes them to the MQTT broker.

FIX: item_count and fill_level are now persisted to /app/data/sensor_state.json
     so a restart does not silently reset the bin to 0 %.
FIX: "emptied" command handler also saves the reset state to disk.
"""
...
def load_state() -> dict:
    """Load persisted sensor state, or return defaults if none exists."""
...
def save_state(state: dict) -> None:
    """Persist sensor state atomically."""
...
def publisher_loop(
    ...
    # FIX: command topic matches what the API publishes
    ...
    def on_connect(client, userdata, flags, reason_code, properties):
        if reason_code == 0:
            ...
            # FIX: subscribe to the exact command topic (not a wildcard with comment mismatch)
    ...
    def on_message(client, userdata, msg):
        ...
        if payload.get("action") == "emptied":
            ...
            with state_lock:
                ...
                state["item_count"] = 0
                state["fill_level"] = 0
                # FIX: persist the reset so a restart does not re-inflate fill level
                save_state(state)
...
def main() -> None:
    ...
    # FIX: load persisted state so item_count / fill_level survive restarts
    state     = load_state()
